...
// --- graph identity ---------------------------------------------------------
// The graph's score keys and node ids are what let the app resolve a tapped
// node back to an exact FINDINGS.md bullet, so they are generated by the real
// CLI hashers rather than asserted against hand-written digests.
const scores = await import(path.join(dist, "governance/scores.js"));
const graphId = await import(path.join(dist, "finding-graph-id.js"));

const graphSamples = [
  { project: "myproj", filename: "FINDINGS.md", snippet: "[pattern] Use the shared cache for repeated lookups" },
  { project: "myproj", filename: "FINDINGS.md", snippet: "Plain untagged finding with enough length" },
  // Same text, different tag — the case score keys exist to disambiguate.
  { project: "myproj", filename: "FINDINGS.md", snippet: "[pitfall] Use the shared cache for repeated lookups" },
  { project: "other-proj", filename: "tasks.md", snippet: "Ship the iOS app" },
  // Whitespace collapsing and the 200-char cap.
  { project: "myproj", filename: "FINDINGS.md", snippet: "[bug]   collapses\n   runs   of\twhitespace" },
  { project: "myproj", filename: "FINDINGS.md", snippet: `[decision] ${"x".repeat(300)}` },
];

writeJson("graph-identity.json", graphSamples.map((sample) => {
  const scoreKey = scores.entryScoreKey(sample.project, sample.filename, sample.snippet);
  return { ...sample, scoreKey, nodeId: graphId.findingStableId(scoreKey) };
}));
