...
\subsection{Migration tooling}
Legacy projects that predate Cortex's structured memory format can import existing knowledge through \texttt{cortex maintain migrate <project>} \cite{cortex-cli}. The migration pipeline scans git history and, when \texttt{gh} is available, GitHub PR reviews, CI failures, and issue comments. Extracted candidates are scored by confidence: high-confidence entries go directly to \texttt{LEARNINGS.md}, while lower-confidence entries are routed to \texttt{MEMORY\_QUEUE.md} for human review. An optional \texttt{--pin} flag promotes entries to canonical status. The \texttt{--dry-run} flag previews what would be extracted without writing any files.

\subsection{Custom integration hooks}
Beyond the three built-in lifecycle stages, Cortex supports user-defined hooks for seven events: \texttt{pre-save}, \texttt{post-save}, \texttt{post-search}, \texttt{pre-learning}, \texttt{post-learning}, \texttt{pre-index}, and \texttt{post-index} \cite{cortex-hooks}. Custom hooks are registered in \texttt{.governance/install-preferences.json} and execute as shell commands with a configurable timeout (default 5s). Each hook receives \texttt{CORTEX\_PATH} and \texttt{CORTEX\_HOOK\_EVENT} as environment variables, plus any event-specific variables passed by the caller. This enables integrations like Slack notifications on new learnings, webhook calls after index rebuilds, or pre-save validation scripts.

\subsection{Data portability}
Projects can be exported as self-contained JSON bundles via \texttt{export\_project} and re-imported with \texttt{import\_project}. The \texttt{manage\_project} tool supports archiving inactive projects (removing them from the active profile without deleting data) and restoring them later. This supports team onboarding, machine migration, and selective backup workflows.
...
Cortex is \textit{local memory + MCP interface + lifecycle hooks}. A local system can still expose MCP tools. The 16 MCP tools cover search, backlog management, learning capture, data portability, and governance \cite{cortex-index}.
...
\begin{itemize}
  ...
  \item MCP tool surface: 16 tools across search, backlog, learning, portability, and governance categories \cite{cortex-index}.
